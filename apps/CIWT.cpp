...
int main(const int argc, const char** argv) {
    ...
    for (int current_frame=CIWTApp::start_frame; current_frame<=CIWTApp::end_frame; current_frame++) {
        ...
        if (CIWTApp::run_tracker) {
            ...
            multi_object_tracker->ProcessFrame(resource_manager, current_frame, left_image.cols, left_image.rows);
        }
        ...
        // cv::Mat left_image_with_corners = left_image.clone();
        ...
        if (CIWTApp::debug_level>=2 || CIWTApp::show_visualization_2d) {
            ...
            // CIWTApp::tracking_visualizer.DrawHypotheses(hypos_to_process_further, left_camera, left_image_with_corners, GOT::tracking::draw_hypos::DrawCroners);
        ...
        if (CIWTApp::debug_level>=2) {
            ...
            if (CIWTApp::run_tracker) {
                ...
                // snprintf(output_path_buff, 500, "%s/hypos_cor_%s.png", output_dir_visual_results.c_str(), frame_str_buff);
                // cv::imwrite(output_path_buff, left_image_with_corners);
            }
        }
