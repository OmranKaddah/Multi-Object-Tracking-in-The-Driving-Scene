import os
import subprocess
from evaluation.comp_rec import run
import datetime
import shutil
import random
import numpy as np
...
def exp(Experiment_name, hyp_param):
    PHASE="training"
    ...
    for DET in DETS:
        ...
        for SEQ in SEQUENCES:
            ...
            args = ["--config", str(DETCFG),
                                ...
                                "--association_score_is_3DIoU", str(0) 
                                , 
...
init(True)
experiment_name = "3d_epx_ped_ori_coup"
pre_mota, pre_motp = exp(experiment_name, hpy)
...
for exp_count in range(50):
    experiment_name = "epx_ped_coup_ori_{}".format(exp_count)
    mota, motp = exp(experiment_name, hpy)
