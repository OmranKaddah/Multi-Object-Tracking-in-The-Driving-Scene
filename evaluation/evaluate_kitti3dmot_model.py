...
class trackingEvaluation(object):
    """ tracking statistics (CLEAR MOT, id-switches, fragments, ML/PT/MT, precision/recall)
             ...
             id-switches - number of id switchess
    ...
    def __init__(self, t_sha, dir, gt_path="./evaluation", model_name = "AB3DMOT", max_truncation = 0, min_height = 25, max_occlusion = 2, mail=None, cls="car", eval_3diou=True, eval_2diou=False, sequence=None):
        ...
        if sequence is None:
            ...
            self.sequence_name = ["%04d" % int(seq) for seq in sequence[0]]
            self.n_frames = sequence[1]
            self.n_sequences = len(sequence[1])
        ...
        self.t_sha             = t_sha
        if model_name[-1] == '*':
            dt_typ = t_sha.split("_")[3]
            self.t_path = os.path.join("./results", dir,model_name[:-1], dt_typ,"data")
            self.cls = t_sha.split("_")[0]
        else:
            self.t_path  = os.path.join("./results", dir,model_name, t_sha, "data")
    ...
    def saveToStats(self, dump, mail,threshold=None, recall=None):
        """
...
class stat:
    """
        Utility class to load data.
    """
    def __init__(self, t_sha, dir, model_name,cls, suffix, dump):
        """
            Constructor, initializes the object given the parameters.
        """
    ...
    def plot(self):
        if self.model_name[-1] == '*':
            dt_typ = self.t_sha.split("_")[3]
            save_dir = os.path.join("./results", self.model_name[:-1], dt_typ)
        else:
            save_dir = os.path.join("./results", self.model_name,self.t_sha)
...
def evaluate(result_sha, dir, model_name, mail,eval_3diou,eval_2diou, sequence = None):
    """
    ...
    if eval_3diou:
        ...
        assert False, 'error'
    classes = []
    returned_eval =  trackingEvaluation(t_sha=result_sha, dir=dir, model_name= model_name,mail=mail,eval_3diou=eval_3diou,eval_2diou=eval_2diou, sequence=sequence)
    avrages = (0,0,0)
    c = result_sha.split("_")[0]
    if c=="ped":
        c = "pedestrian"
    elif c=="cyc":
        c = "cyclist"
    # for c in ("car"):
    e = trackingEvaluation(t_sha=result_sha, dir=dir, model_name= model_name,mail=mail,cls=c,eval_3diou=eval_3diou,eval_2diou=eval_2diou, sequence=sequence)
    ...
    try:
        if e.loadTracker():
            ...
            stat_meter = stat(t_sha=result_sha, dir=dir, model_name = model_name, cls=c, suffix=suffix, dump=dump)
            ...
            for threshold_tmp, recall_tmp in zip(threshold_list, recall_list):
                ...
                if mota_tmp > best_mota: 
                    ...
                    best_mota = mota_tmp
                e.saveToStats(dump, mail,threshold_tmp, recall_tmp) 

            e.reset()
            e.compute3rdPartyMetrics(best_threshold)
            returned_eval = e
            e.saveToStats(dump, mail) 

            stat_meter.output()
            avrages = (stat_meter.sAMOTA, stat_meter.amota, stat_meter.amotp)
    ...
    return True, returned_eval, avrages
...
if __name__ == "__main__":
    ...
    # evaluate results
    success, _  = evaluate(result_sha, model_name,mail,eval_3diou,eval_2diou)
