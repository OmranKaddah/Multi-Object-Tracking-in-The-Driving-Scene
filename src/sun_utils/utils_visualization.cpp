...
namespace SUN {
    namespace utils {
        namespace visualization {
            ...
            void DrawDots(const Eigen::Vector3d &p1, const SUN::utils::Camera &camera, cv::Mat &ref_image, const cv::Scalar &color, int thickness, int line_type, const cv::Point2i &offset) {
                if (p1[2] <0.1)
                    return;
                ...
                // Draw line, but only if both end-points project into the image!
                if (p1_in_bounds) { // This is correct. Won't draw only if both lines are out of bounds.
                    // Draw line
                    auto p1_offs = offset+cv_p1;

                    cv::circle(ref_image, p1_offs, 2.0,color, thickness, line_type);
