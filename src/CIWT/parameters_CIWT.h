...
namespace CIWTApp {
    void InitParameters(po::options_description &options) {
        ...
        options.add_options()
                ...
                ("not_selected_tolerance", po::value<int>()->default_value(10), "tolerence of how many frames the hypo is not selected.")
                ...
                ("report_extrapolated", po::value<bool>()->default_value(true), "Whehter to report extrapolated(with no corresponding detection) track")
                ("check_exit_to2D_projeciton", po::value<bool>()->default_value(true), "tracks termination if the projection of 3D bbox is completely out of image frame.")
                ("corners_left", po::value<int>()->default_value(1), "Keep the track once # of corner left in the image")
                ...
                ///  --- Data Association ---
                ("association_score_is_3DIoU", po::value<bool>()->default_value(false), "Use 3D IoU for association score, instead of the defauly method in CIWT")
                ("association_appearance_model_weight", po::value<double>()->default_value(0.4), "Data association: Appearance model weight.")
                ("IoU3D_model_weight", po::value<double>()->default_value(0.8), "IoU3D wieght for association, IoU2D would be in this case 1-IoU3D weight.")
                ...
                ("gaiting_min_association_score", po::value<double>()->default_value( 0.428  /*0.01*/), "Data association (gaiting):  Total assoc. score threshold.")
                ...
                ("kf_2d_observation_noise", po::value<double>()->default_value(1.0), "KF-2D: Obs. noise")
                ("kf_2d_initial_variance", po::value<double>()->default_value(10.0), "KF-2D: Initial variance")
                ("kf_2d_system_noise", po::value<double>()->default_value(1.0), "KF-2D: System noise")
                ("kf_3d_observation_noise" ,po::value<double>()->default_value(1.0), "KF-3D: Obs. noise")
                ("kf_3d_initial_variance", po::value<double>()->default_value(10.0), "KF-3D: Initial variance")
                ("kf_3d_system_noise", po::value<double>()->default_value(1.0), "KF-3D: System noise")

                ("kf_init_velocity_variance_car_x", po::value<double>()->default_value(10000.0), "KF-3D: initial velocity variance (X).")
                ("kf_init_velocity_variance_car_z", po::value<double>()->default_value(10000.0), "KF-3D: initial velocity variance (Z).")
                ("kf_system_pos_variance_car_x", po::value<double>()->default_value(1.0), "KF-3D: pos. sys. variance (X).")
                ("kf_system_pos_variance_car_z", po::value<double>()->default_value(1.0), "KF-3D: pos. sys. variance (Z).")
                ...
                ("kf_orientation_obs_noise", po::value<double>()->default_value(0.05), "KF obeservation noise.")
                
                ("is_oriented", po::value<bool>()->default_value(true), "Model rotationY in the hypo")
                ("enable_coupling", po::value<bool>()->default_value(true), "Enable 2D-3D state coupling")
