...
// CIWT
#include "CIWT_dynamics_handler_orientation.h"
...
namespace GOT {
    namespace tracking {
        ...
        // -------------------------------------------------------------------------------
        void DynamicsHandlerOrientationCIWT::InitializeState(const SUN::utils::Camera &camera, const Observation &obs, bool forward, Hypothesis &hypo) {

            // Create filter object
            OrientedCoupledImageWorldSpaceFilter::Parameters params;
            ...
            hypo.kalman_filter() = std::shared_ptr<OrientedCoupledImageWorldSpaceFilter>(new OrientedCoupledImageWorldSpaceFilter(params));
            ...
            // orientation 
            // No variable created for it, since ni modifications are required
            ...
            if (use_3d_obs_process) {
                ...
                // Init state
                // [x,y,dx,dy,w,h,d | x_b2d, y_b2d, w_b2d, h_b2d, rotationY, dx_b2d, dy_b2d]
                ...
                kalman_init_state << footpoint_world[0], footpoint_world[2],
                        ...
                        w_3d, h_3d, d_3d,
                        reparam_bbox_2d,
                        obs.rotationY(), // rotationY/orientation
                        0.0, 0.0,
                        0.0, 0.0; 
        ...
        void DynamicsHandlerOrientationCIWT::ApplyTransition(const SUN::utils::Camera &camera, const Eigen::VectorXd &u, Hypothesis &hypo) {
...
//                kf_state_after_projection[12] = 10;
            ...
            kf_state_after_projection[15] = 0.0;

            // orientation correction
            if (kf_state_after_projection[11] >= M_PI)
                kf_state_after_projection[11] -= M_PI * 2.0;
        ...
        void DynamicsHandlerOrientationCIWT::ApplyCorrection(const SUN::utils::Camera &camera, const Observation &obs, bool forward, Hypothesis &hypo) {
            ...
            bool got_velocity = !(std::isnan(obs.velocity()[0]));
            bool use_3d_obs_process = (obs.footpoint()[2] < this->max_dist_use_stereo_) && obs.proposal_3d_avalible() && got_velocity;
            // make sure that the angle of the predcited orientation and measured one is acute

            double predicted_theta = hypo.kalman_filter()->x()[11];
            double new_theta = obs.rotationY();

            if ((abs(new_theta - predicted_theta) > M_PI / 2.0) && (abs(new_theta - predicted_theta) < M_PI * 3 / 2.0)){                
                predicted_theta += M_PI;       
                if (predicted_theta >= M_PI) 
                    predicted_theta-= M_PI * 2.0;    // make the theta still in the range
                if (predicted_theta < -M_PI)
                    predicted_theta += M_PI * 2.0;
            }
            // now the angle is acute: < 90 or > 270, convert the case of > 270 to < 90
            if (abs(new_theta - predicted_theta) >= M_PI * 3 / 2.0){                
                if (new_theta > 0) 
                    predicted_theta += M_PI * 2;
                else
                    predicted_theta -= M_PI * 2;
            }
            hypo.kalman_filter()->SetRotationY(predicted_theta);
            ...
            if (use_3d_obs_process) {
                ...
                H_tmp.setZero(12,16);
                H_tmp.block(0,0,12,12) = Eigen::MatrixXd::Identity(12, 12);
                ...
                R.block(5,5,4,4) = Eigen::MatrixXd::Identity(4,4)*bounding_box_obs_noise_;
                R(9,9) = orientation_variance_obs_;
                measurement.setZero(10);
                measurement << footpoint_world[0], footpoint_world[2], w_3d, h_3d, d_3d, reparam_bbox_2d, new_theta; // bbox_cx, bbox_cy, bbox_w, bbox_h;
                hypo.kalman_filter()->Correction(measurement, R, H_observe_pos_bbox2d_size_);
            }
            predicted_theta = hypo.kalman_filter()->x()[11];
            if (predicted_theta >= M_PI)
                predicted_theta-= M_PI * 2.0;
        ...
        void DynamicsHandlerOrientationCIWT::InitializeMatrices() {

            /// Initial uncertainty: Pedestrian
            P0_pedestrian_.setZero(state_dim_,state_dim_);
            P0_pedestrian_(0, 0) = 10.0; // Pose
            P0_pedestrian_(1, 1) = 10.0;
            P0_pedestrian_(2, 2) = P0_ped_velocity_variance_[0]; // Velocity
            P0_pedestrian_(3, 3) = P0_ped_velocity_variance_[1];
            P0_pedestrian_(4, 4) = kf_3d_init;
            P0_pedestrian_(5, 5) = kf_3d_init;
            P0_pedestrian_(6, 6) = kf_3d_init;
            P0_pedestrian_.block(7,7,9, 9) = bounding_box_initial_variance_*bounding_box_initial_variance_*Eigen::MatrixXd::Identity(9, 9);
            P0_pedestrian_(11,11)= orientation_variance_obs_;
            /// Initial uncertainty: Car
            P0_car_.setZero(state_dim_,state_dim_);
            P0_car_(0, 0) = 10.0; // Pose
            P0_car_(1, 1) = 10.0;
            P0_car_(2, 2) = P0_car_velocity_variance_[0]; // Velocity
            P0_car_(3, 3) = P0_car_velocity_variance_[1];
            P0_car_(4, 4) = kf_3d_init;
            P0_car_(5, 5) = kf_3d_init;
            P0_car_(6, 6) = kf_3d_init;
            P0_car_.block(7,7,9, 9) = bounding_box_initial_variance_*bounding_box_initial_variance_*Eigen::MatrixXd::Identity(9, 9);
            P0_car_(11,11) = orientation_variance_obs_;
            /// System noise: Pedestrian
            G_pedestrian_.setZero(state_dim_,state_dim_);
            G_pedestrian_(0, 0) = G_ped_pos_variance_[0];
            G_pedestrian_(1, 1) = G_ped_pos_variance_[1];
            G_pedestrian_(2, 2) = G_ped_velocity_variance_[0];
            G_pedestrian_(3, 3) = G_ped_velocity_variance_[1];
            G_pedestrian_(4, 4) = kf_3d_sys; // w
            G_pedestrian_(5, 5) = kf_3d_sys; // h
            G_pedestrian_(6, 6) = kf_3d_sys; // l
            Eigen::MatrixXd bbox_system_noise_matrix;
            bbox_system_noise_matrix.setIdentity(9,9);
            bbox_system_noise_matrix *= bounding_box_system_noise_;
            G_pedestrian_.block(7,7, 9, 9) = bbox_system_noise_matrix;
            G_pedestrian_(11,11) = orientation_variance_obs_;

            /// System noise: Car
            G_car_.setZero(state_dim_,state_dim_);
            G_car_(0, 0) = G_car_pos_variance_[0];
            G_car_(1, 1) = G_car_pos_variance_[1];
            G_car_(2, 2) = G_car_velocity_variance_[0];
            G_car_(3, 3) = G_car_velocity_variance_[1];
            G_car_(4, 4) = kf_3d_sys; // w
            G_car_(5, 5) = kf_3d_sys; // h
            G_car_(6, 6) = kf_3d_sys; // l
            G_car_.block(7,7, 9, 9) = bbox_system_noise_matrix;
            G_car_(11,11) = orientation_variance_obs_;


            /// Partial observation
            H_observe_pos_bbox2d_size_.setZero(10, 16);
            H_observe_pos_bbox2d_size_.block(0,0,2,2) = Eigen::Matrix2d::Identity();
            H_observe_pos_bbox2d_size_.block(2,4,8,8) = Eigen::MatrixXd::Identity(8,8);

            /// Full-observation
            H_observe_everything_.setZero(14,16);
            H_observe_everything_.block(0,0,14,14) = Eigen::MatrixXd::Identity(14,14);
        }

        DynamicsHandlerOrientationCIWT::DynamicsHandlerOrientationCIWT(const po::variables_map &params) : DynamicsModelHandler(params) {
            state_dim_ = 16;
            ...
            kf_3d_obs = params.at("kf_3d_observation_noise").as<double>();
            kf_3d_init = params.at("kf_3d_initial_variance").as<double>();
            kf_3d_sys = params.at("kf_3d_system_noise").as<double>();
            ...
            orientation_variance_obs_ = params.at("kf_orientation_obs_noise").as<double>();
            P0_car_velocity_variance_ = Eigen::Vector2d (kf_init_vel_x, kf_init_vel_z);
            P0_ped_velocity_variance_ = Eigen::Vector2d (kf_init_vel_x, kf_init_vel_z);
            ...
            G_ped_pos_variance_ = Eigen::Vector2d (kf_sys_pos_x, kf_sys_pos_z);
            G_ped_velocity_variance_ = Eigen::Vector2d (kf_sys_vel_x, kf_sys_vel_z);
