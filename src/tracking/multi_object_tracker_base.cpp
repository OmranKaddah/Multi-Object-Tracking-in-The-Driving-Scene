...
namespace GOT {
    namespace tracking {
        ...
        void MultiObjectTracker3DBase::CheckExitTo2DProjeciton(const SUN::utils::Camera &camera, int current_frame, int &img_width, int &img_height) {
            Eigen::VectorXd bbox_3d;
            Eigen::Vector3d temp;


            for (auto &hypo:hypotheses_) {
                // computing the oriented 3D box.
                bbox_3d = hypo.bounding_boxes_3d().back();
                Eigen::Vector4d center;
                center.head(3) = bbox_3d.head(3);
                center[3] = 1.0;
                ...
                //End of computing the oriented 3D box.

                // projecting 3D box to image.
                int toRemove = 0;
                for(int ix= 0; ix<8; ++ix){
                    temp = Eigen::Map<Eigen::Vector3d>(box3d.col(ix).data(),3);
                    if (temp[2] <0.0 || temp[2] <0.0)
                        continue;
                    Eigen::Vector4d p1_4d;
                    p1_4d[3] = 1.0;
                    p1_4d.head<3>() = temp;
                    Eigen::Vector3i projected_point_1 = camera.CameraToImage(p1_4d);
                    auto cv_p1 = cv::Point2i(projected_point_1[0],projected_point_1[1]);

                    if (!(cv_p1.x > 0) && (cv_p1.y > 0) && (cv_p1.x < img_width) && (cv_p1.y < img_height) )
                    {
                        toRemove += 1;
                        // if (toRemove > 8-parameter_map_.at("corners_left").as<int>())
                        //     break;
                    }
        ...
        std::vector<Hypothesis> MultiObjectTracker3DBase::RemoveInactiveHypotheses(const std::vector<Hypothesis> &hypotheses,
            ...
            //     const int last_detection_frame_v2 =   hypo.at(i).set_last_frame_selected(frame);

            //     if (std::abs(hypo.last_frame_selected() - frame) <=
            //         max_hole_size &&
            //         std::abs(frame - last_detection_frame)  <=
            //         max_hole_size
            //         && !hypo.terminated())
            //         filtered_set.push_back(hypo);
            //     else{

            //         if (this->verbose_ && !hypo.terminated())
            //             std::cout << "Hypo " << hypo.id() << " received no detections for " << max_hole_size << " frames, terminated." << std::endl << std::endl;
 
            //         else 
            //             std::cout << "Hypo " << hypo.id() << " is terminated "<< std::endl << std::endl;
